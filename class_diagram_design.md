...
package "web.controller" {
    class UserController <<controller>> {
        + createUser(dto: CreateUserRequestBodyDto): ResponseEntity
        + getUser(id: Integer): ResponseEntity
        + getCurrentUser(): ResponseEntity
    }
    class MigrantController <<controller>> {
        + createMigrant(dto: CreateMigrantRequestBodyDto): ResponseEntity
        + getMigrant(id: Integer): ResponseEntity
        + updateMigrant(dto: UpdateMigrantRequestBodyDto): ResponseEntity
        + deleteMigrant(id: Integer): ResponseEntity
        + getCurrentMigrant(): ResponseEntity
    }
    class RoadMapController <<controller>> {
        + showRoadMap(): ResponseEntity
    }
    class RuleController <<controller>> {
        + create(dto: CreateRuleRequestBodyDto): ResponseEntity
        + get(id: Integer): ResponseEntity
        + getRules(onlyActive: boolean): ResponseEntity
        + update(dto: UpdateRuleRequestBodyDto): ResponseEntity
        + delete(id: Integer): ResponseEntity
    }
    class ReferenceController <<controller>> {
        + getCountries(): ResponseEntity
        + getPurposesOfVisit(): ResponseEntity
    }
}
...
' --- Мапперы -> сущности и DTO ---
UserMapper ..> User
UserMapper ..> UserResponseDto
MigrantMapper ..> Migrant
MigrantMapper ..> MigrantResponseDto
RuleMapper ..> Rule
RuleMapper ..> RuleResponseDto
RoadMapMapper ..> RoadMap
RoadMapMapper ..> RoadMapResponseDto
ReferenceMapper ..> CountryResponseDto
ReferenceMapper ..> PurposeOfVisitResponseDto

' --- Вложенность DTO и ссылки на Operators ---
CreateRuleRequestBodyDto "1" *-- "1..*" RuleConditionDto : conditions
UpdateRuleRequestBodyDto "1" *-- "0..*" RuleConditionDto : conditions
RuleResponseDto "1" *-- "0..*" ConditionResponseDto : conditions
RoadMapResponseDto "1" *-- "0..*" StepResponseDto : steps
RuleConditionDto --> Operators : operator
ConditionResponseDto --> Operators : operator
...
Все классы, включая DTO, показаны с полным набором полей из кода. У `UpdateMigrantRequestBodyDto` булевы статусы — обёртки `Boolean` (null = «поле не меняем» при частичном PATCH), у `CreateMigrantRequestBodyDto` и `MigrantResponseDto` — примитивные `boolean`. Вложенные списки DTO (`conditions`, `steps`) показаны композициями.
